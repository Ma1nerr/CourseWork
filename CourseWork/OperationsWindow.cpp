...
// Реалізація методів сортування залишається без змін...
void OperationsWindow::on_sortColorAscButton_clicked()
{
    if (cars.isEmpty()) {
        QMessageBox::warning(this, "Попередження", "Немає даних для сортування.");
        return;
    }
...
// ВИПРАВЛЕНІ МЕТОДИ АНАЛІЗУ:

void OperationsWindow::on_findBrandButton_clicked()
{
    if (cars.isEmpty()) {
        QMessageBox::warning(this, "Попередження", "Немає даних для пошуку.");
        return;
    }
    ...
    QString brand = QInputDialog::getText(this, "Пошук за маркою",
                                          ...
                                          QLineEdit::Normal, "", &ok);
    if (!ok || brand.isEmpty()) {
        showResult("Пошук скасовано", "Операцію пошуку скасовано.");
        return;
    }
    ...
    // ВИПРАВЛЕНА ЛОГІКА: знаходимо автомобілі, які мають одночасно мінімальну ціну та мінімальну потужність
...
{
    if (cars.isEmpty()) {
        QMessageBox::warning(this, "Попередження", "Немає даних для аналізу.");
        return;
    }
...
void OperationsWindow::on_priceRangeButton_clicked()
{
    ...
    if (!ok1 || !ok2) {
        showResult("Діапазон цін", "Введення діапазону скасовано.");
        return;
    }

    if (minPrice > maxPrice) {
        QMessageBox::warning(this, "Помилка", "Мінімальна ціна не може бути більшою за максимальну.");
        return;
    }
